...
// requested service and characteristic
func (b *ClientAdaptor) Subscribe(cUUID string, indicate bool, f func([]byte, error)) (err error) {
	if !b.connected {
	...
	if u := b.profile.Find(blelib.NewCharacteristic(uuid)); u != nil {
		...
		err = b.client.Subscribe(u.(*blelib.Characteristic), indicate, h)
		if err != nil {
			return err
		}
		return nil
